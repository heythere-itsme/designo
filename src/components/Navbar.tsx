  ...
  return (
    <div className='grid grid-cols-2 lg:grid-cols-3 p-6 fixed left-0 right-0 top-0 z-50'>
        <div className='flex items-center gap-4'>
            ...
            </Link>
            {
                !hasCanvas || (!hasStyleGuide && (
                    <div className='lg:inline-block hidden rounded-full text-primary/60 border border-white/[0.12] backdrop-blur-xl bg-white/[0.08] text-sm saturate-150'>
        ...
        <div className='lg:flex hidden items-center justify-center gap-2'>
            <div className='flex items-center gap-2 backdrop-blur-xl bg-white/[0.08] border border-white/[0.12] rounded-full p-2 saturate-150'>
                {tabs.map((t) => (
                    ...
                    key={t.href}
                    href={t.href}
                    className={['group inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm transition', `${pathname}?project=${projectId}` == t.href ? 'bg-white/[0.12] text-white border border-white/[0.16] backdrop-blur-sm' : 'text-zinc-400 hover:text-zinc-200 border border-transparent hover:bg-white/[0.06]'].join(' ')}>
                        <span
        ...
        <div className='flex items-center gap-4 justify-end'>
            <span className='text-sm text-white/50'>Credit: 05</span>
            <Button variant="secondary" className='rounded-full h-12 w-12 flex items-center justify-center backdrop-blur-xl border border-white/[0.12] bg-white/[0.08] saturate-150 hover:bg-white/[0.12]'>
